...
public class Sonar extends Device {
    ...
    private SonarMeasurement detectMeasure;
    ...
    public void  executeCommand(String command)
    {
        if (command.contains("ROTATETO")) {
            ...
            SonarMeasurement measure;
            String measures = "SCAN";
            for (SonarMeasurement scanMeasure : sonarMeasurement) {
                measure = scanMeasure;
    ...
    public void nextStep()
    {
        ...
        if (detect) {
            ...
            if (distance > -1.0) {
                sonarMeasurement.add(new SonarMeasurement(distance, localPosition.getT()));  // ??????????????
            }
        }
    }
